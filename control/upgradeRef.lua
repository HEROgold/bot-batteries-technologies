                ...
                for i = 1, logistics_point.sections_count do
                    ...
                    for ii = 1, section.filters_count do
                        ...
                        if
                            ...
                            and slot.value ~= nil
                            and slot.value.name
                                == "alien-hyper-module-" .. storage.currentmodulelevel[forceName] - 1
                        then
                            local current_filter = section.get_slot(ii)
                            section.set_slot(ii, {
                                value = {
                                    name = "alien-hyper-module-" .. storage.currentmodulelevel[forceName],
                                    quality = current_filter.value.quality,
                                    comparator = current_filter.value.comparator,
                                },
        ...
        elseif entityType == "player" then
            ...
            if entity.cursor_stack ~= nil and entity.cursor_stack.valid_for_read then
                if string.find(entity.cursor_stack.name, "^alien%-hyper%-module") then
                    ...
                    if
                        tonumber(string.match(entity.cursor_stack.name, "%d+$")) < storage.currentmodulelevel[forceName]
                    then
                        ...
                        entity.cursor_stack.set_stack({
                            name = "alien-hyper-module-" .. math.min(storage.currentmodulelevel[forceName], 100),
                            count = old_count,
                            quality = old_quality,
                ...
                if string.find(entity.cursor_stack.name, "^alien%-hyper%-magazine") then
                    ...
                    then
                        ...
                        entity.cursor_stack.clear() --clear the slot
                        entity.cursor_stack.set_stack({
                            name = "alien-hyper-magazine-" .. math.min(storage.currentmodulelevel[forceName], 100),
                            count = old_count,
                            quality = old_quality,
                        }) --add the updated level modules with whatever amount we recorded
                    end
                            ...
                            inventory.set_filter(
                                i,
                                {
                                    name = "alien-hyper-module-"
                                        .. math.min(storage.currentmodulelevel[forceName], 100),
                                    quality = old_quality,
                                }
                            ) --update filter
                        end
                ...
                if string.find(inventory[i].name, "^alien%-hyper%-magazine") then
                    ...
                    if tonumber(string.match(inventory[i].name, "%d+$")) < storage.currentmodulelevel[forceName] then
                        ...
                        if entityType == "player" and inventory.get_filter(i) ~= nil then
                            ...
                            inventory.set_filter(
                                i,
                                "alien-hyper-magazine-" .. math.min(storage.currentmodulelevel[forceName], 100)
                            ) --update filter
                        end
    ...
    for _, entity in ipairs(assemblers) do
        if entity.get_recipe() ~= nil then
            ...
            if string.find(entity.get_recipe().name, "^alien%-hyper%-module") then
                ...
                local finished_module_count = entity
                    .get_inventory(defines.inventory.assembling_machine_output)
                ...
                if finished_module_count > 0 then
                    entity.get_inventory(defines.inventory.assembling_machine_output).insert({
                        name = "alien-hyper-module-" .. storage.currentmodulelevel[force.name],
            ...
            if
                settings.startup["alien-module-hyper-ammo-enabled"].value
                and string.find(entity.get_recipe().name, "^alien%-hyper%-magazine")
            then
                local finished_module_count = entity
                    .get_inventory(defines.inventory.assembling_machine_output)
                    .get_item_count("alien-hyper-module-" .. storage.currentmodulelevel[force.name] - 1)
    ...
    for index, module_on_ground in pairs(modulesOnGround) do
        ...
        if string.find(real_name, "^alien%-hyper%-module") and real_name ~= current_module_name then
            ...
            surface.create_entity({
                name = "item-on-ground",
                position = module_pos,
                stack = { name = current_module_name, count = item_count },
            })
        end

        if
            settings.startup["alien-module-hyper-ammo-enabled"].value
            and (string.find(real_name, "^alien%-hyper%-magazine") and real_name ~= current_magazine_name)
        then
            module_on_ground.destroy()
            surface.create_entity({
                name = "item-on-ground",
                position = module_pos,
                stack = { name = current_magazine_name, count = item_count },
            })
        end
    end
end
        ...
        if not storage.ignoredalienmodulefactions[force.name] then
            ...
            if storage.modulelevel[forceName] > storage.currentmodulelevel[forceName] then
                ...
                for _, surface in pairs(game.surfaces) do
                    local assemblers =
                        surface.find_entities_filtered({ force = forceName, type = "assembling-machine" })
                    ...
                    local logisticChests =
                        surface.find_entities_filtered({ force = forceName, type = "logistic-container" })
