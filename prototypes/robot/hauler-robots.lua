...
local function create_hauler_robot(c, s, e)
	...
	local name = "hauler-robot-mk-" .. suffix

	robot_entity.localised_name = { "entity-name.hauler-robot-mk-", tostring(c), tostring(s), tostring(e) }
	robot_item.localised_name = { "item-name.hauler-robot-mk-", tostring(c), tostring(s), tostring(e) }
	...
	robot_entity.max_payload_size = c + hauler_entity.max_payload_size * c * modifier_max_payload_size:get()
	robot_entity.speed = s + hauler_entity.speed * s * modifier_speed:get()

	-- Use Energy class for proper energy manipulation
	local base_energy = Energy.new(hauler_entity.max_energy)
	local energy_multiplier = e * modifier_max_energy:get()
	local scaled_energy = Energy.new(hauler_entity.max_energy)
